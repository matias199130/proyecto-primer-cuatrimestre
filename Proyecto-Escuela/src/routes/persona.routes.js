...
router.get("/add", (req, res) => {
  res.render("personas/add", { success: req.flash("success")});
});
...
router.post("/add", async (req, res) => {
  try {
    ...
    req.flash("success", "Se ha agregado una nueva persona");
    console.log("Mensaje de éxito:", req.flash("success"));
    //redirecciona a la pagina principal de personas despues de enviar el formulario
    res.redirect("/list");
  } catch (err) {
...
router.get("/delete/:id", async (req, res) => {
  try {
   ...
   req.flash("warning", "La persona fue eliminada")
   res.redirect("/list");
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
